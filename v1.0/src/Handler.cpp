    ...
    switch (key.code)
    {
        case Keyboard::Key::Escape:
            if(auto state = dynamic_cast<PausedState*>(game_state.state.get())) {
                state->resume();
                game_state.state = std::move(std::make_unique<RunningState>(game_state));
            }
            else if(dynamic_cast<RunningState*>(game_state.state.get()))
                game_state.state = std::move(std::make_unique<PausedState>(game_state));
...
void Handler::handleEnterPressed(GameState& game_state)
{
    ...
    else if(dynamic_cast<RunningState*>(game_state.state.get()))
    {
        ...
        if(game_state.player.dialogueActive)
        {
            ...
            game_state.state = std::move(std::make_unique<DialogState>(game_state, npc));
        }
    }
